...
if ( ! class_exists( 'Jet_Blog_Tools' ) ) {
	...
	class Jet_Blog_Tools {
		...
		public function get_carousel_arrow( $icon = '', $arrow = 'next' ) {
			...
			return sprintf( $format, $this->prepare_arrow( $icon ), $arrow );
		}
		 ...
		 * @return array
		 */
		public function get_fa5_arrows_map() {
			return apply_filters(
				'jet-blog/fa5-arrows-map',
				array(
		 ...
		 * @return string
		 */
		public function prepare_arrow( $arrow ) {

			if ( ! $this->is_fa5_migration() ) {
				return $arrow;
			}

			$fa5_arrows_map = $this->get_fa5_arrows_map();
