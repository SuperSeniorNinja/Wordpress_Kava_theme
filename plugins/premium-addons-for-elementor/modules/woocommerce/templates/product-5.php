			...
			foreach ( $product_structure as $index => $segment ) {
				...
				switch ( $value ) {
					...
					case 'cta':
						...
						if ( 'yes' === $quick_view && 'button' === $qv_type ) {
							echo '<div class="premium-woo-qv-btn button" data-product-id="' . esc_attr( $product_id ) . '"><i class="premium-woo-qv-icon fa fa-eye"></i></div>';
						}
						echo '</div>';
						break;
