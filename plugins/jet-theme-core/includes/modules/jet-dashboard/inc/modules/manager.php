...
class Manager {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'init_modules' ), 999 );
	}
	...
	public function load_page_module( $page_module ) {
		...
		$page_module_data = $this->get_page_module( $page_module );

		if ( ! $page_module_data ) {
			return;
		}

		$page_module_class = $page_module_data['class'];
		$page_module_args = isset( $page_module_data['args'] ) ? $page_module_data['args'] : array();

		$page_module_instance = new $page_module_class( $page_module_args );
