...
if ( ! class_exists( 'CX_Db_Updater' ) ) {
	...
	class CX_Db_Updater {
		...
		public function __construct( $args = array() ) {
			...
			$this->messages = array(
				'error'   => esc_html__( 'Module DB Updater init error in %s - version and slug is required arguments', 'jet-cw' ),
				'update'  => esc_html__( 'We need to update your database to the latest version.', 'jet-cw' ),
				'updated' => esc_html__( 'Update complete, thank you for updating to the latest version!', 'jet-cw' ),
			);
		...
		private function notice_submit( $slug = '' ) {
			...
			$label  = esc_html__( 'Start Update', 'jet-cw' );
			$url    = add_query_arg(
				array(
		...
		private function notice_title( $slug ) {
			...
			printf( '<strong>%1$s %2$s</strong> &#8211; ', $name, esc_html__( 'Data Update', 'jet-cw' ) );
		}
