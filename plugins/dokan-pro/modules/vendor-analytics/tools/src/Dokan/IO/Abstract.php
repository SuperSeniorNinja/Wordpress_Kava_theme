...
abstract class Dokan_IO_Abstract
{
  ...
  {
    $this->client = $client;
    $timeout = $client->getClassConfig('Dokan_IO_Abstract', 'request_timeout_seconds');
    if ($timeout > 0) {
  ...
  public function setCachedRequest(Dokan_Http_Request $request)
  {
    ...
    if (Dokan_Http_CacheParser::isResponseCacheable($request)) {
  ...
  public function makeRequest(Dokan_Http_Request $request)
  {
  	...
  	// Add support for dokan_analytics Endpoint
  	do_action('dokan_analytics_endpoint_support', $request);

    // First, check to see if we have a valid cached version.
    $cached = $this->getCachedRequest($request);
    if ($cached !== false && $cached instanceof Dokan_Http_Request) {
  ...
  {
    if (false === Dokan_Http_CacheParser::isRequestCacheable($request)) {
      return false;
    }
  ...
  {
    if (Dokan_Http_CacheParser::mustRevalidate($cached)) {
      $addHeaders = array();
