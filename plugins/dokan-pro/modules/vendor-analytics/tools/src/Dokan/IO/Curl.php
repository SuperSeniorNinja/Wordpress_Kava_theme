...
/**
 * Curl based implementation of Dokan_IO.
 *
...
class Dokan_IO_Curl extends Dokan_IO_Abstract
{
  ...
  public function executeRequest(Dokan_Http_Request $request)
  {
    ...
    $options = $this->client->getClassConfig('Dokan_IO_Curl', 'options');
    if (is_array($options)) {
    ...
    if ($response === false) {
      ...
      $map = $this->client->getClassConfig('Dokan_IO_Exception', 'retry_map');

      $this->client->getLogger()->error('cURL ' . $error);
      throw new Dokan_IO_Exception($error, $code, null, $map);
    }
  ...
  protected function needsQuirk()
  {
    ...
    return $versionNum < Dokan_IO_Curl::NO_QUIRK_VERSION;
  }
}
