...
abstract class Dokan_Logger_Abstract
{
  ...
  {
    $this->setLevel(
        $client->getClassConfig('Dokan_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('Dokan_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('Dokan_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'Dokan_Logger_Abstract',
        'allow_newlines'
    );
  }
