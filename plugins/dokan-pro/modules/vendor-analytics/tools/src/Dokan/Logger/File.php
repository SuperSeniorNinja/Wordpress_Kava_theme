...
class Dokan_Logger_File extends Dokan_Logger_Abstract
{
  ...
  public function __construct(Dokan_Client $client)
  {
    ...
    $file = $client->getClassConfig('Dokan_Logger_File', 'file');
    if (!is_string($file) && !is_resource($file)) {
      throw new Dokan_Logger_Exception(
          'File logger requires a filename or a valid file pointer'
      );
    }

    $mode = $client->getClassConfig('Dokan_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('Dokan_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new Dokan_Logger_Exception('File pointer is no longer available');
    }
