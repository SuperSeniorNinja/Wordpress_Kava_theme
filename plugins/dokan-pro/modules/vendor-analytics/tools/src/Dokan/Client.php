...
class Dokan_Client
{
  ...
  /**
   * @var Dokan_Auth_Abstract $auth
   */
  private $auth;

  /**
   * @var Dokan_IO_Abstract $io
   */
  private $io;

  /**
   * @var Dokan_Cache_Abstract $cache
   */
  private $cache;

  /**
   * @var Dokan_Config $config
   */
  private $config;

  /**
   * @var Dokan_Logger_Abstract $logger
   */
  private $logger;
  ...
  public function __construct($config = null)
  {
    if (is_string($config) && strlen($config)) {
      $config = new Dokan_Config($config);
    } else if ( !($config instanceof Dokan_Config)) {
      $config = new Dokan_Config();

      if ($this->isAppEngine()) {
        // Automatically use Memcache if we're in AppEngine.
        $config->setCacheClass('Dokan_Cache_Memcache');
      }

      if (version_compare(phpversion(), "5.3.4", "<=") || $this->isAppEngine()) {
        // Automatically disable compress.zlib, as currently unsupported.
        $config->setClassConfig('Dokan_Http_Request', 'disable_gzip', true);
      }
    }

    if ($config->getIoClass() == Dokan_Config::USE_AUTO_IO_SELECTION) {
      if (function_exists('curl_version') && function_exists('curl_exec')
          && !$this->isAppEngine()) {
        $config->setIoClass("Dokan_IO_Curl");
      } else {
        $config->setIoClass("Dokan_IO_Stream");
      }
    }
  ...
  public function loadServiceAccountJson($jsonLocation, $scopes)
  {
    ...
    if (isset($data->type) && $data->type == 'service_account') {
      ...
      $cred = new Dokan_Auth_AssertionCredentials(
          $data->client_email,
          $scopes,
          $data->private_key
      );
      ...
      throw new Dokan_Exception("Invalid service account JSON file.");
    }
  }
  ...
  public function setAuthConfig($json)
  {
    ...
    if (!isset($data->$key)) {
      throw new Dokan_Exception("Invalid client secret JSON file.");
    }
  ...
  public function prepareScopes()
  {
    if (empty($this->requestedScopes)) {
      throw new Dokan_Auth_Exception("No scopes specified");
    }
  ...
  {
    $auth = new Dokan_Auth_OAuth2($this);
  ...
  {
    if ($request instanceof Dokan_Http_Request) {
      $request->setUserAgent(
          $this->getApplicationName()
          . " " . self::USER_AGENT_SUFFIX
          . $this->getLibraryVersion()
      );
      if (!$this->getClassConfig("Dokan_Http_Request", "disable_gzip")) {
        $request->enableGzip();
      }
      $request->maybeMoveParametersToBody();
      return Dokan_Http_REST::execute($this, $request);
    } else if ($request instanceof Dokan_Http_Batch) {
      return $request->execute();
    } else {
      throw new Dokan_Exception("Do not know how to execute this type of object.");
    }
  }
