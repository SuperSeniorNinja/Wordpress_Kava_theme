...
class Dokan_Task_Runner
{
  ...
  /**
   * @var Dokan_Client $client The current API client.
   */
  private $client;
  ...
  public function __construct(
    ...
    $config = (array) $client->getClassConfig('Dokan_Task_Runner');

    if (isset($config['initial_delay'])) {
      if ($config['initial_delay'] < 0) {
        throw new Dokan_Task_Exception(
  ...
  public function run()
  {
    while ($this->attempt()) {
      ...
      } catch (Dokan_Task_Retryable $exception) {
