...
class IpfsConnector
{
    ...
    public function uploadCustomImage($name, $description, $file_path) {
        if (file_exists($file_path)) {
            if(filesize($file_path) > self::MAX_FILE_SIZE_BYTES) {
                throw new \Exception('IPFS: Image is too big.');
            }
            $content = file_get_contents($file_path);
            $image = ['name' => $name, 'content' => $content];
            $responseImage = self::storeIpfsFile($image);
            $json = self::createMetadataJson($image, rawurldecode($responseImage['ipfsHash']), $description);
            $responseMetadata = self::storeIpfsFile(array('name' => 'metadata.json', 'content' => $json));
            return rawurldecode($responseMetadata['ipfsHash']);
        }
        throw new \Exception('IPFS: Cannot find image.');
    }
