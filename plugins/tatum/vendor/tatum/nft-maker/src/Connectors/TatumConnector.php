...
class TatumConnector
{
    ...
    private function get($url, $base = null, $apiKey = null) {
        $args = array('headers' => self::headers($apiKey), 'timeout' => 20);
    ...
    private function headers($apiKey = null): array {
        return array('Accept' => 'application/json', 'Content-Type' => 'application/json', 'x-api-key' => $apiKey ?? $this->apiKey);
    }

    public function getApiVersion($apiKey = null) {
        try {
            $eu = $this->get('/v3/tatum/version', Constants::TATUM_URL['eu1'], $apiKey);
        ...
        try {
            $us = $this->get('/v3/tatum/version', Constants::TATUM_URL['us1'], $apiKey);
    ...
    public function callScMethod($chain, $body) {
        return $this->post('/v3/' . $chain . '/smartcontract', $body);
    }

    public function getNftTokensByAddress($chain, $address) {
        return $this->get('/v3/nft/address/balance/' . $chain . '/' . $address);
    }

    public function getNftTokenBalance($chain, $address, $contractAddress) {
        return $this->get('/v3/nft/balance/' . $chain . '/' . $contractAddress . '/' . $address);
    }

    public function getNftTokenMetadata($chain, $tokenId, $contractAddress) {
        return $this->get('/v3/nft/metadata/' . $chain . '/' . $contractAddress . '/' . $tokenId);
    }
}