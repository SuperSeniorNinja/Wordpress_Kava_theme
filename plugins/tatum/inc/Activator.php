...
class Activator
{
    ...
    public function dbDelta($errorlevel) {
        ...
        $explorer = $this->getTableName("explorer");
        $sql = "CREATE TABLE IF NOT EXISTS $explorer (
            id bigint NOT NULL AUTO_INCREMENT,
            tokenId varchar(256) NOT NULL,
            smartContractAddress varchar(256) NOT NULL,
            address varchar(256) NOT NULL,
            chain ENUM('CELO', 'ETH', 'BSC', 'ONE', 'MATIC', 'XDC') NOT NULL,
            UNIQUE KEY id (id),
            UNIQUE KEY nft (tokenId, smartContractAddress, chain, address)
        ) $charset_collate;";
        $wpdb->query($sql);
    }
