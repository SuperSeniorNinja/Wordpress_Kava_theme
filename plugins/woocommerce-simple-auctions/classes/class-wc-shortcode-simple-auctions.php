 ...
 */
class WC_Shortcode_Simple_Auctions {
	 ...
	 */
	public function __construct( $attributes = array(), $type = 'auctions' ) {
		$this->type       = $type;
	...
	protected function parse_attributes( $attributes ) {
		...
		$attributes = shortcode_atts(
			array(
				...
				'auction_status'          => '',      // Should shortcode output be cached.
				'future'          => '',      // Should shortcode output be cached
				'meta_key'          => '',      // Should shortcode output be cached.
			),
			$attributes,
			$this->type
		);
	...
	protected function parse_query_args() {
		...
		$this->set_auctions_query_args( $query_args );
		$this->set_auctions_status_query_args( $query_args );
		$this->set_auctions_future_query_args( $query_args );
		
		$query_args['is_auction_archive'] = true;
		$query_args['auction_arhive'] = true;

		$query_args = apply_filters( 'woocommerce_shortcode_auctions_query', $query_args, $this->attributes, $this->type );
	 ...
	 * @param array $query_args Query args.
	 */
	protected function set_auctions_query_args( &$query_args ) {
	...
	protected function set_auctions_status_query_args( &$query_args ) {
		...
		if ( ! empty( $this->attributes['auction_status'] ) ) {

			if ( $this->attributes['auction_status'] === 'all' ){

				$query_args['show_past_auctions'] = true;
				$query_args['show_future_auctions'] = true;

			} elseif( $this->attributes['auction_status'] === 'finished' ) {
			...
			} elseif( $this->attributes['auction_status'] === 'future' ) {
				$query_args['meta_query'] []=   array(
								'key'     => '_auction_closed',
								'compare' => 'NOT EXISTS',
								);

				$query_args['meta_query'] []=  array( 'key' => '_auction_started', 'value'=> '0' );
				$query_args['show_future_auctions'] = true;

			} elseif($this->attributes['auction_status'] === 'active'){
	 ...
	 * 
	 */
	protected function set_auctions_future_query_args( &$query_args ) {

		if ( ! empty( $this->attributes['future'] ) ) {

			if ( $this->attributes['future'] === 'yes' || $this->attributes['future'] === 'true' ){

				$query_args['show_future_auctions'] = true;

			} elseif( $this->attributes['future'] === 'false' ) {
	 ...
	 * 
	 */
	protected function set_my_winned_auctions_query_args( &$query_args ) {
	...
	protected function get_query_results() {
		...
		if ( 1==2 && isset( $transient_value['value'], $transient_value['version'] ) && $transient_value['version'] === $transient_version ) {
	...
	/**
	 * Loop over found products / auctions.
	 *
	 * @return string
	 * 
	 */
