...
class WC_SA_Widget_Recent_Auction extends WP_Widget {
	...
	function __construct() {
		...
		$this->woo_widget_cssclass = 'woocommerce widget_recent_auctions';
		$this->woo_widget_description = esc_html__( 'Display a list of your most recent auctions on your site.', 'woocommerce' );
		$this->woo_widget_idbase = 'woocommerce_recent_auctions';
		$this->woo_widget_name = esc_html__( 'WooCommerce Recent Auctions', 'woocommerce' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		parent::__construct('recent_auctions', $this->woo_widget_name, $widget_ops);
	...
	function widget($args, $instance) {
		...
		$cache = wp_cache_get('widget_recent_auctions', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('New Auctions', 'woocommerce' ) : $instance['title'], $instance, $this->id_base);
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['widget_recent_auctions']) ) delete_option('widget_recent_auctions');

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete('widget_recent_auctions', 'widget');
	}
	...
	function form( $instance ) {
        ...
        ?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e( 'Title:', 'woocommerce' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of auctions to show:', 'woocommerce' ); ?></label>
