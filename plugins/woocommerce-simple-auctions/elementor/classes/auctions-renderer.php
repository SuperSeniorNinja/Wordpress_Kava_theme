...
class Auctions_Renderer extends Base_Auctions_Renderer {
	...
	protected function parse_query_args() {
		...
		if ( $ordering_args['orderby'] === 'bid' ){
			if ( isset ( $ordering_args['order'] ) ) {
				$ordering_args['orderby'] = $ordering_args['order'] == 'ASC' ? 'bid_asc' : 'bid_desc';
			}
		}
	...
	protected function set_auctions_status_query_args( &$query_args ) {
			...
			if ( ! empty( $settings[ 'query_post_type' ] ) ) {
				if( $settings[ 'query_post_type' ] === 'finished'|| $settings['show_finished'] == 'yes'){
				...
				} elseif( $settings[ 'query_post_type' ] === 'future' || $settings['show_future'] == 'yes' ){
						 $query_args['meta_query'] []=   array(
											'key'     => '_auction_closed',
											'compare' => 'NOT EXISTS',
											);

						$query_args['meta_query'] []=  array( 'key' => '_auction_started',
													'value'=> '0',);
