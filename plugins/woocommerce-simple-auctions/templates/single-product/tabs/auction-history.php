...
<?php if(($product->is_closed() === TRUE ) and ($product->is_started() === TRUE )) : ?>
	...
	if ( $product->get_auction_closed() == '3' ){ ?>
		...
		<p><?php esc_html_e('Product sold for buy now price', 'wc_simple_auctions') ?>: <span><?php echo wp_kses ( wc_price( $product->get_regular_price() ), true ); ?></span></p>

	<?php } elseif ( $product->get_auction_current_bider()){ ?>

		<p><?php esc_html_e('Highest bidder was', 'wc_simple_auctions') ?>: <span><?php echo apply_filters( 'woocommerce_simple_auctions_displayname', get_userdata( $product->get_auction_current_bider())->display_name, $product ); ?></span></p>
...
<table id="auction-history-table-<?php echo esc_attr( $product->get_id() ); ?>" class="auction-history-table">
    <?php 
        ...
        <?php if ( $product->is_sealed() ){
                ...
                echo "<tr>";
                echo "<td colspan='4'  class='sealed'>".esc_html__('This auction is sealed. Upon auction finish auction history and winner will be available to the public.', 'wc_simple_auctions')."</td>";
                echo "</tr>"; 
            ...
            foreach ($auction_history as $history_value) {
                ...
                echo "<td class='username'>". apply_filters( 'woocommerce_simple_auctions_displayname', get_userdata($history_value->userid)->display_name, $product ) . "</td>";
	...
	<tr class="start">
        ...
        if ($product->is_started() === TRUE ){
            ...
            echo apply_filters('auction_history_started_text', esc_html__( 'Auction started', 'wc_simple_auctions' ), $product);
            echo '</td>';
                ...
                echo apply_filters('auction_history_starting_text', esc_html__( 'Auction starting', 'wc_simple_auctions' ), $product);
                echo '</td>' ;
        }
        ?>
	</tr>
